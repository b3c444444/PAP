...
<?php include "menu.php" ?>
  ...
  <h1>Gerador de QR Code</h1>
    <div id="qrcode"></div>
    <input type="text" id="idservidor" placeholder="IP do Servidor">
    <input type="text" id="idpessoa" placeholder="Id da Pessoa">
    <button onclick="gerarQRCode()">Gerar QR Code</button>
    <button onclick="imprimirQRCode()">Imprimir QR Code</button> 
    <button onclick="gerarQRCodeDoBanco()">Gerar QR Code da BD</button>

    <script>
        function gerarQRCodeDoBanco() {
            $.ajax({
                url: 'generateALL.php',
                method: 'GET',
                dataType: 'json',
                ...
                error: function(error) {
                    console.error("Erro na solicitação AJAX:", error.responseText);
                }
            });
        ...
        function gerarQRCode() {
            var texto = "http://" + document.getElementById("idservidor").value + "/PAP/FinalProduct/get_medicalinfos.php?id=" + document.getElementById("idpessoa").value;
            var qrcodeContainer = document.getElementById("qrcode");
